...
//            int recipeIndexChange = GuiBase.isShiftDown() ? recipesPerPage : recipesPerPage / 2;
            ...
            if (input.key() >= ScanCodes.SCAN_KP_1 && input.key() <= ScanCodes.SCAN_KP_9)
            {
                index = Mth.clamp(input.key() - ScanCodes.SCAN_KP_1, 0, 8);
            }
            else if (input.key() == ScanCodes.SCAN_UP && oldIndex > 0)
            {
                index = oldIndex - 1;
            }
            else if (input.key() == ScanCodes.SCAN_DOWN && oldIndex < (recipes.getTotalRecipeCount() - 1))
            {
                index = oldIndex + 1;
            }
            else if (input.key() == ScanCodes.SCAN_LEFT && oldIndex >= recipeIndexChange)
            {
                index = oldIndex - recipeIndexChange;
            }
            else if (input.key() == ScanCodes.SCAN_RIGHT && oldIndex < (recipes.getTotalRecipeCount() - recipeIndexChange))
            {
                index = oldIndex + recipeIndexChange;
            }
    ...
    {
	    return this.handleInput(ScanCodes.SCAN_UNKNOWN, false, amount);
    }

    @Override
    public boolean onMouseClick(MouseButtonEvent click, boolean eventButtonState)
    {
	    return this.handleInput(click.input() - ScanCodes.OFFSET_MOUSE, eventButtonState, 0);
    }

    private boolean handleInput(int scanCode, boolean keyState, double dWheel)
    {
        ...
        boolean cancel = this.handleInputImpl(scanCode, keyState, dWheel, mc);
    ...
    private boolean handleInputImpl(int scanCode, boolean keyState, double dWheel, Minecraft mc)
    {
        ...
        {
            final boolean isAttack = InputUtils.isAttack(scanCode, mc);
            final boolean isUse = InputUtils.isUse(scanCode, mc);
            final boolean isPickBlock = InputUtils.isPickBlock(scanCode, mc);
